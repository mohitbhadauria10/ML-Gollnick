---
title: "Multivariate Regression Solution"
output: 
  html_document:
    toc: true
    toc_float: true
...
corrplot::corrplot.mixed(cor_airfoil)
```

Which two variables correlate most to target variable?

```{r}
cor_airfoil[, 6] %>% sort %>% head(n = 2)
```
...
```{r}
model_fit <- lm(formula = spl ~ ., data = airfoil_noise)
```
...
caret::postResample(pred = airfoil_noise$spl_pred, obs = airfoil_noise$spl)
```
...
```{r}
model_fit_freq_displacement_chord <- lm(formula = spl ~ frequency + ss_displacement_thickness + chord_length, 
                                        data = airfoil_noise)
```
...
```{r}
# create predictions
airfoil_noise$spl_pred <- predict(object = model_fit_freq_displacement_chord, newdata = airfoil_noise)

# perform model performance
caret::postResample(pred = airfoil_noise$spl_pred, 
...
```{r}
"No. Rsquared is significantly lower."
```
